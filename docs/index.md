---
layout: home
title: Agent Herder — Scale from One to Many Claude Code Sessions
description: You don't browse the web in a single tab. Why use Claude Code in a single session? Agent Herder scales you from one to many parallel sessions — creating 10x devs from anyone with Claude Code chops.

hero:
  name: Agent Herder
  text: Scale from one to many
  tagline: "You don't browse the web in a single tab. Why use Claude Code in a single session? Agent Herder gives you massively parallel Claude Code — creating 10x devs from anyone with Claude Code chops."
  actions:
    - theme: brand
      text: Get Started
...
features:
  ...
  - title: Use Claude Code like a tabbed browser
    details: You don't visit one page at a time. You open tabs. Agent Herder brings the same workflow to Claude Code — open, switch, fork, close sessions as naturally as browser tabs.
